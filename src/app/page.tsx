      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={triggerSeed}
            disabled={seeding}
            className="px-4 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-200 text-sm font-semibold rounded-lg border border-slate-700 transition-colors shadow-sm cursor-pointer"
          >
            {seeding ? "Seeding..." : "Seed Dummy Data"}
          </button>
          <Link
            href="/invoices?new=true"
            className="px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white text-sm font-semibold rounded-lg shadow-md hover:shadow-indigo-500/20 transition-all flex items-center space-x-1 cursor-pointer"
          >
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900/40 border border-slate-800/80 rounded-xl p-6 relative overflow-hidden group hover:border-slate-700/80 transition-colors">
                  ...
                  <div className="text-xs text-slate-300 space-y-1.5 font-medium">
                    {riskyClients.map((client) => (
                      <p key={client.clientId}>
                        • Klien{" "}
                        <Link href="/clients" className="text-amber-400 underline hover:text-amber-300">
                          {client.name}
                        </Link>{" "}
                        dinilai berisiko{" "}
          ...
          <div className="bg-slate-900/30 border border-slate-900 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/invoices" className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold underline">
                Lihat Semua Invoice
              </Link>
            </div>
                ...
                <table className="min-w-full divide-y divide-slate-800/80">
                  ...
                  <tbody className="divide-y divide-slate-800/60 text-sm">
                    {recentInvoices.map((inv) => (
                      <tr key={inv.invoiceId} className="hover:bg-slate-900/20 transition-colors">
